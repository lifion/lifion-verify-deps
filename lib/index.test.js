...
describe('lib/index', () => {
  ...
  test('should upgrade upto patch versions when versions prefixed with ~', async () => {
    ...
    jest.mock('foo/node_modules/foo1/package.json', () => ({ version: '46.45.44' }), {
      virtual: true
    });
  ...
  test.only('should upgrade upto minor versions when versions prefixed with ^', async () => {
    jest.mock(
      `bar/package.json`,
      () => ({ dependencies: { foo1: '^46.45.44' }, devDependencies: {} }),
      { virtual: true }
    );
    jest.mock('bar/node_modules/foo1/package.json', () => ({ version: '46.45.44' }), {
      virtual: true
    });
    ...
    await expect(verifyDeps({ dir: 'bar', logger })).rejects.toThrow(
      'Please update your installed modules'
    );
  ...
  test('should compare installed dependencies to latest NPM versions', async () => {
    mockExports.version = '1.0.0';
    mockExports.dependencies = { jugageni: '^1.0.0', soawitut: '^1.0.0' };
    mockExports.devDependencies = {};
    mockExecAsync
    ...
    expect(mockExecAsync).toHaveBeenCalledWith(`npm view soawitut versions --json`);
    expect(mockJoin).toHaveBeenCalledWith(dir, 'package.json');
    expect(mockJoin).toHaveBeenCalledWith(dir, 'node_modules', 'jugageni', 'package.json');
    expect(mockJoin).toHaveBeenCalledWith(dir, 'node_modules', 'soawitut', 'package.json');
  });

  test('should compare installed devDpendencies to latest NPM versions', async () => {
    mockExports.version = '1.0.0';
    mockExports.dependencies = {};
    mockExports.devDependencies = { jugageni: '^1.0.0', soawitut: '^1.0.0' };
    ...
    expect(mockExecAsync).toHaveBeenCalledWith(`npm view soawitut dist-tags --json`);
    expect(mockJoin).toHaveBeenCalledWith(dir, 'package.json');
    expect(mockJoin).toHaveBeenCalledWith(dir, 'node_modules', 'jugageni', 'package.json');
    expect(mockJoin).toHaveBeenCalledWith(dir, 'node_modules', 'soawitut', 'package.json');
  });
  ...
  test('should show dependency update required when using semver and later version in range is available', async () => {
    mockExports.version = '1.0.0';
    mockExports.dependencies = { suji: '^1.0.0', tohepu: '^1.0.0' };
    mockExports.devDependencies = { ju: '^1.0.0', waz: '^1.0.0' };
  ...
  test('should not show dependency update required when using semver and later version is out of range', async () => {
    mockExports.version = '1.0.0';
    mockExports.dependencies = { mi: '^1.0.0', nawpo: '^1.0.0' };
    mockExports.devDependencies = { di: '^1.0.0', koptigu: '^1.0.0' };
  ...
  test('should show dependency update required when version is locked if non-major-version update available', async () => {
    mockExports.version = '1.0.0';
    mockExports.dependencies = { fu: '^1.0.0', lezejujuk: '^1.0.0' };
    mockExports.devDependencies = { kuvrib: '^1.0.0', voliki: '^1.0.0' };
    ...
    mockExecAsync
      ...
      .mockImplementationOnce(() => Promise.resolve({ stdout: JSON.stringify(['1.0.0']) }))
      .mockImplementationOnce(() =>
        Promise.resolve({ stdout: JSON.stringify(['1.0.0', '1.1.0']) })
      );
  ...
  test('should not show dependency update required when version is locked if only major-version update available', async () => {
    mockExports.version = '1.0.0';
    mockExports.dependencies = { an: '^1.0.0', nagte: '1.0.0' };
    mockExports.devDependencies = { pu: '^1.0.0', tedo: '1.0.0' };
  ...
  test('should show dependency install required if module cannot be found', async () => {
    mockExports.version = '1.0.0';
    mockExports.dependencies = { eri: '^1.0.0', unapup: '^1.0.0' };
    mockExports.devDependencies = { ok: '^1.0.0', zousga: '^1.0.0' };

    mockJoin.mockImplementation((...args) => {
      if (args[2] === 'eri') throw new Error('module not found');
      return pathString;
    });
    ...
    expect(logger.info).toHaveBeenNthCalledWith(
      2,
      'Error getting a list of installed modules from package.json - Error: module not found'
    );
  ...
  test('throw error when getting latest tag fails', async () => {
    ...
    await expect(verifyDeps({ dir, logger })).rejects.toThrow(
      `Error getting latest tag - Error: foo1`
    );
  });

  test('should show dependency install required if latest module is installed but not reflected in package.json', async () => {
    mockExports.version = '1.0.0';
    mockExports.dependencies = { lule: '^1.0.1', zew: '^1.0.0' };
    mockExports.devDependencies = { neh: '^1.0.1', pe: '^1.0.0' };
    ...
    // TODO: Handle this case, to avoid misleading messages
    expect(logger.info).toHaveBeenCalledTimes(5);
    expect(logger.info).toHaveBeenNthCalledWith(1, 'Verifying dependencies…\n');
    expect(logger.info).toHaveBeenNthCalledWith(2, `lule is outdated: 1.0.1 → 1.0.1`);
    expect(logger.info).toHaveBeenNthCalledWith(3, `neh is outdated: 1.0.1 → 1.0.1`);
    expect(logger.info).toHaveBeenNthCalledWith(4, `\n${'To resolve this, run:'}`);
  ...
  test('should not throw an error if no dependencies are in package.json', async () => {
    mockExports.version = '1.0.0';
    mockExports.dependencies = undefined;
    mockExports.devDependencies = undefined;
    await verifyDeps({ dir, logger });
    expect(logger.info).toHaveBeenCalledTimes(2);
    expect(logger.info).toHaveBeenNthCalledWith(1, 'Verifying dependencies…\n');
    expect(logger.info).toHaveBeenNthCalledWith(2, 'All NPM modules are up to date.');
  ...
  test('should upgrade pre-release versions to latest pre-release version available', async () => {
    mockExports.version = '1.0.0-alpha.0';
  ...
  test('autoUpgrade modules', async () => {
    mockExports.version = '1.0.0';
    mockExports.dependencies = { oli: '^1.0.0', upe: '^1.0.0' };
    mockExports.devDependencies = { barda: '^1.0.0', gugkih: '^1.0.0' };
  ...
  test('throw error when npm module name is invalid', async () => {
    ...
    mockExports.devDependencies = {};
    mockExecAsync.mockImplementationOnce(() => Promise.resolve({ stdout: ['1.2.3'] }));
