...
describe('lib/index', () => {
  ...
  it('should show dependency install required if module cannot be found', async () => {
    ...
    expect(logger.info).toHaveBeenNthCalledWith(3, `${outdatedDep} is ${'not installed'}`);
    expect(logger.info).toHaveBeenNthCalledWith(
      4,
      `${outdatedDevDep} is outdated: ${olderVersion} → ${newerVersion}`
    );
  ...
  it('should upgrade pre-release versions when includePreReleaseDependencies flag is set to true', async () => {
    mockExports.dependencies = { foo1: '1.0.0-alpha' };
    mockExports.devDependencies = { fooDev2: '1.2.3-beta' };
    ...
    mockExecAsync
      ...
      .mockImplementationOnce(() => Promise.resolve({ stdout: JSON.stringify(['1.2.4']) }))
      .mockImplementationOnce(() => Promise.resolve({ stdout: JSON.stringify(['1.2.4']) }));
    await verifyDeps({ autoUpgrade: true, dir, includePreReleaseDependencies: true, logger });

    expect(logger.info).toHaveBeenCalledTimes(7);
    expect(logger.info).toHaveBeenNthCalledWith(1, 'Verifying dependencies…\n');
    expect(logger.info).toHaveBeenNthCalledWith(2, `foo1 is outdated: 1.0.0-alpha → 1.2.4`);
    expect(logger.info).toHaveBeenNthCalledWith(3, `fooDev2 is outdated: 1.2.3-beta → 1.2.4`);
    expect(logger.info).toHaveBeenNthCalledWith(4, 'UPGRADING…');
    expect(logger.info).toHaveBeenNthCalledWith(5, `npm i foo1@1.2.4 \nnpm i -D fooDev2@1.2.4 `);
    expect(logger.info).toHaveBeenNthCalledWith(6, `Upgraded dependencies:\n["1.2.4"]`);
    expect(logger.info).toHaveBeenNthCalledWith(7, `Upgraded development dependencies:\n["1.2.4"]`);
  });

  it('should not upgrade pre-release versions when includePreReleaseDependencies flag false by default', async () => {
    mockExports.dependencies = { foo1: '1.0.0-alpha' };
    mockExports.devDependencies = { fooDev2: '1.2.3-beta' };
    ...
    mockExecAsync
      .mockImplementationOnce(() => Promise.resolve({ stdout: JSON.stringify(['1.2.4', '1.2.5']) }))
      .mockImplementationOnce(() =>
        Promise.resolve({ stdout: JSON.stringify(['1.2.4', '1.2.5']) })
      );
    await verifyDeps({ autoUpgrade: true, dir, logger });
