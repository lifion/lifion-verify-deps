...
describe('lib/index', () => {
  ...
  it('should show dependency update required when using semver and later version in range is available', async () => {
    ...
    expect(logger.info).toHaveBeenNthCalledWith(1, 'Checking NPM module versions…\n');
  ...
  it('should default to native console when no logger is passed', async () => {
    ...
    expect(console.info).toHaveBeenNthCalledWith(1, 'Checking NPM module versions…\n');
  ...
  test('autoUpgrade modules', async () => {
    ...
    mockExecAsync.mockImplementationOnce(mock);
    mockExecAsync.mockImplementationOnce(mock);
    mockExecAsync.mockImplementationOnce(mock2);
    mockExecAsync.mockImplementationOnce(mock2);
    ...
    expect(logger.info).toHaveBeenNthCalledWith(6, `Upgraded PROD Packages:\n["1.0.0"]`);
    expect(logger.info).toHaveBeenNthCalledWith(7, `Upgraded Dev Packages:\n["1.0.0"]`);
  });
});
