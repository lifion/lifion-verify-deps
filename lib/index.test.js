...
describe('lib/index', () => {
  ...
  test('should show dependency update required when using semver and later version in range is available', async () => {
    ...
    expect(logger.info).toHaveBeenNthCalledWith(
      2,
      `suji is outdated: ${olderVersion} → ${newerVersion}`
    );
    expect(logger.info).toHaveBeenNthCalledWith(
      3,
      `ju is outdated: ${olderVersion} → ${newerVersion}`
    );
  ...
  test('should not upgrade when no version available for major version of pre-release', async () => {
    mockExports.version = '1.0.0-alpha.0'; // TODO: check other tests This is installed version
    mockExports.dependencies = { foo1: '^1.0.0-alpha.0' };
    mockExports.devDependencies = {};
  ...
  test('autoUpgrade modules', async () => {
    ...
    expect(logger.info).toHaveBeenNthCalledWith(
      2,
      `oli is outdated: ${olderVersion} → ${newerVersion}`
    );
    expect(logger.info).toHaveBeenNthCalledWith(
      3,
      `barda is outdated: ${olderVersion} → ${newerVersion}`
    );
    expect(logger.info).toHaveBeenNthCalledWith(4, 'UPGRADING…');
